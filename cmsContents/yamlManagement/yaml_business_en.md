      ...
      parameters:
        ...
        X-Use-Account:
          ...
          schema:
            type: string
          description: |
            API 요청 처리시 사용될 *LGE Members Platform* 계정의 출처를 지정합니다.

              |Value|Description|
              |-|-|
              |REGISTERED|My Page에서 이미 등록한 계정|
              |IN-HEADER|이 HTTP 요청의 `Authorization`` 헤더에 OAuth 토큰으로 지정된 계정 (**현재 ThinQ 등록 가전제품에 대해서만 지원합니다.**)|
          example: REGISTERED
        Authorization:
          name: Authorization
          in: header
          schema:
            type: string
          description: |
            ThinQ 서비스에 가입한 LGE Members Platform 계정의 `Bearer` OAuth 토큰.  `X-Use-Account` 헤더가 HTTP 요청에 포함되어 있으면서 헤더의 값이 `IN-HEADER`인 경우, 이 헤더는 필수적으로 요청에 포함되어야 합니다.
        ...
        X-Conditional-Control:
          ...
          description: |
            ...
            `X-Use-Account` 헤더가 HTTP 요청에 포함되어 있으면서 헤더의 값이 `IN-HEADER`인 경우, 이 헤더는 필수적으로 요청에 포함되어야 합니다.
          example: true
        Authorization-2:
          name: Authorization
          in: header
          schema:
            type: string
          description: |
            ThinQ 서비스에 가입한 *LGE Members Platform* 계정의 `Bearer` OAuth 토큰
          example: Bearer 5a9a713f51a95c53d781addd1af0dfa4f6e1e7420a8bff3c5198308dac571aa9845832b8d29bbe1f04deec2d35229c6d
        X-Country-Code-2:
          name: X-Country-Code
          in: header
