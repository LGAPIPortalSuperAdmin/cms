      ...
      - name: Push API
        description: |
          특정 디바이스의 푸시 알림을 B2B 파트너의 서비스가 수신하거나 수신을 해제하기 위해 Push API를 사용합니다.  이 API는 현재 LG ThinQ 등록 디바이스에 한해 제공되며 향후 다른 디바이스의 지원을 확대할 예정입니다.  B2B 파트너의 서비스가 디바이스의 상태를 수신하기 위해서는 사전에 LG Smart Solution API Developer에서 B2B 파트너 서비스의 Callback 호출 정보를 등록해야 합니다.  Callback으로 제공하는 푸시 알림의 종류는 아래와 같고, 종류별로 Callback 구독이 수행하는 조건이 다를 수 있습니다.
           ...
           |-|-|-|
           | DEVICE_PUSH | 디바이스의 동작 완료, 디바이스 부품 교체 등을 알림<br> (예: 세탁 완료, 필터 교체 등) | [디바이스 푸시 메시지 구독 API](#tag/Push-API/operation/subscribePushMessages) 호출 |
           | DEVICE_REGISTERED | 디바이스가 추가됨 | [디바이스 목록 조회 API](#tag/Device-API/operation/getDevices) 호출 |
           | DEVICE_UNREGISTERED | 디바이스가 삭제됨 | [디바이스 목록 조회 API](#tag/Device-API/operation/getDevices) 호출 |
           | DEVICE_ALIAS_CHANGED | 디바이스의 닉네임이 변경됨 | [디바이스 목록 조회 API](#tag/Device-API/operation/getDevices) 호출 |
      ...
      - name: Event API
        description: |
          특정 디바이스의 상태가 변경되었을 때 변경된 항목에 대한 메시지를 B2B 파트너의 서비스가 수신하거나 수신을 해제하기 위해 Event API를 사용합니다.  이 API는 현재 LG ThinQ 등록 가전제품에 한해 제공되며 향후 다른 디바이스의 지원을 확대할 예정입니다.  B2B 파트너의 서비스가 디바이스의 상태를 수신하기 위해서는 사전에 LG Smart Solution API Developer에서 B2B 파트너 서비스의 Callback 호출 정보를 등록해야 합니다. B2B 파트너가 이 API를 사용하기 위해서는 LG전자의 계약 담당자와의 협의가 필요합니다.
           ...
           | Push Type | Description | Pre-Condition |
           |-|-|-|
           | DEVICE_STATUS | 디바이스의 변경된 상태를 알림 | [디바이스 이벤트 구독 API](#tag/Event-API/operation/subscribeEventMessages) 호출 |
      ...
      - name: DR API
        description: |
          ...
          ### 2. DR Request 발령
            ...
            - LG전자는 등록된 DR 이벤트를 지정된 시작 시간 이전에 대상 디바이스에 전달합니다.
    ...
    paths:
      ...
      /devices:
        get:
          ...
          responses:
            ...
            '401':
              description: Unauthorized (인증 오류인 경우 응답 본문(Body)은 제공되지 않습니다.)
              headers:
                X-Response-Code:
                  schema:
                    type: string
      ...
      /event:
        get:
          ...
          description: |
            이벤트 메시지를 구독한 디바이스의 ID 목록을 조회합니다. **(현재 LG ThinQ 등록 가전제품만 지원합니다.)**
      ...
      /event/{deviceId}/subscribe:
        post:
          ...
          summary: 디바이스 이벤트 구독
          description: |
            특정 디바이스에서 전달하는 이벤트 메시지를 구독합니다. **(현재 LG ThinQ 등록 가전제품만 지원합니다.)**
      ...
      /event/{deviceId}/unsubscribe:
        delete:
          ...
          summary: 디바이스 이벤트 해제
          description: |
            특정 디바이스에서 전달하는 이벤트 메시지의 구독을 해제합니다. **(현재 LG ThinQ 등록 가전제품만 지원합니다.)**
              ...
              properties:
                event:
                  ...
                  properties:
                    ...
                    report:
                      type: object
                      description: 디바이스의 변경된 상태입니다. 디바이스 타입별 상태 응답 스키마는 [**디바이스 프로파일**](/ko/apiManage/device_profile) 페이지를 참조해주세요.
                    userList:
                      type: array
                      description: 이벤트를 구독한 사용자 번호의 목록입니다.
                      items:
                        type: string
                        example: n2uR0D5vLjvZRvTdvhVJ
      parameters:
